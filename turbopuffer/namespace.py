...
from turbopuffer.vectors import (Cursor, VectorColumns, VectorResult,
                                 VectorRow, batch_iter)
...
class Namespace:
    ...
    @overload
    def upsert(
        self, data: Union[dict, VectorColumns], distance_metric: Optional[str] = None
    ) -> None:
        """
        Creates or updates multiple vectors provided in a column-oriented layout.
        If this call succeeds, data is guaranteed to be durably written to object storage.
    ...
    @overload
    def upsert(self, data: VectorResult, distance_metric: Optional[str] = None) -> None:
        """
    ...
    def force_index(self, distance_metric: str) -> None:
        """
        Forces a re-index of the entire dataset. turbopuffer will automatically
        build indexes, but this function can be used after e.g. a large batch.
        """
        ...
        assert (
            response.get("content", dict()).get("status", "") == "accepted"
        ), f"Invalid force_index() response: {response}"
