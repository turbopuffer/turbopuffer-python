    ...
    try:
        ...
        for file_path in parquet_files:
            ...
            if MAX_OFFSET is not None and file_offset > MAX_OFFSET:
                thread_print(
                    f"Skipping remaining files, file_offset from {file_offset} to {file_offset + file_size - 1}"
                )
                break
            elif file_offset + file_size <= START_OFFSET:
                thread_print(
                    f"Skipped {file_path}, file_offset from {file_offset} to {file_offset + file_size - 1}"
                )
                file_offset += file_size
            else:
                # Add any attribute columns to include after 'emb'
                for i in parquet_file.iter_batches(
                    batch_size=tpuf.upsert_batch_size, columns=["emb"]
                ):
                    ...
                    if file_offset + len(df) <= START_OFFSET:
                        thread_print(
                            f"Skipped batch file_offset from {file_offset} to {file_offset + len(df) - 1}"
                        )
                        file_offset += len(df)
                        continue
                    elif MAX_OFFSET is not None and file_offset > MAX_OFFSET:
                        thread_print(
                            f"Skipping remaining files, file_offset from {file_offset} to {file_offset + len(df) - 1}"
                        )
                        break
                    ...
                    while not exiting.is_set():
                        try:
                            ...
                            pass
                    thread_print(
                        f"Loaded {file_path}, file_offset from {file_offset} to {file_offset + len(df) - 1}"
                    )
                    file_offset += len(df)
    except Exception:
...
def upsert_docs_from_queue(input_queue, dataset_name, exiting):
    ...
    while batch is not None and not exiting.is_set():
        try:
            ...
            thread_print(
                f"Completed {batch['id'][0]}..{batch['id'][batch.shape[0]-1]} time: {time_diff} / {len(batch)} = {len(batch)/time_diff}"
            )
        except Exception:
            thread_print(
                f"Failed to upsert batch: {batch['id'][0]}..{batch['id'][batch.shape[0]-1]}"
            )
            traceback.print_exc()
        batch = input_queue.get()
...
def main(dataset_name, input_path):
    ...
    read_thread = threading.Thread(
        target=read_docs_to_queue, args=(doc_queue, sorted_files, exiting)
    )
    upsert_threads = []
    ...
    try:
        ...
        for _ in range(NUM_THREADS):
            upsert_thread = threading.Thread(
                target=upsert_docs_from_queue, args=(doc_queue, dataset_name, exiting)
            )
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        thread_print(
            f"Usage: {sys.argv[0]} <dataset_name> <input_folder>\n"
