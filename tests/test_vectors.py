...
def test_upsert_rows():
    ...
    ns.upsert(
        tpuf.VectorRow(id=i, vector=[i / 10, i / 10], attributes={"test": "rows"})
    ...
    assert results[1] == tpuf.VectorRow(
        id=7, vector=[0.7, 0.7], attributes={"hello": "world"}
    )
    for i in range(10, 100):
        assert results[i - 8] == tpuf.VectorRow(
            id=i, vector=[i / 10, i / 10], attributes={"test": "rows"}
        )
...
def test_delete_vectors():
    ...
    assert results[0] == tpuf.VectorRow(
        id=7, vector=[0.7, 0.7], attributes={"hello": "world"}
    )
    for i in range(16, 100):
        assert results[i - 15] == tpuf.VectorRow(
            id=i, vector=[i / 10, i / 10], attributes={"test": "rows"}
        )
...
def test_upsert_columns():
    ...
    ns.upsert(ids=ids, vectors=vectors, attributes=attributes)
...
def test_query_vectors():
    ...
    expected = [
        tpuf.VectorRow(
            id=7, vector=[0.7, 0.7], attributes={"hello": "world"}, dist=0.01
        ),
    ...
    expected = [
        tpuf.VectorRow(
            id=7, vector=[0.7, 0.7], dist=0.01, attributes={"hello": "world"}
        ),
        tpuf.VectorRow(
            id=10, vector=[1.0, 1.0], dist=0.13, attributes={"test": "cols"}
        ),
        tpuf.VectorRow(
            id=11, vector=[1.1, 1.1], dist=0.25, attributes={"test": "cols"}
        ),
        tpuf.VectorRow(
            id=3,
            vector=[0.3, 0.3],
            dist=0.41,
            attributes={"test": "cols", "key1": "three", "key2": "c"},
        ),
        tpuf.VectorRow(
            id=6,
            vector=[0.3, 0.3],
...
def test_list_vectors():
    ...
    assert set_str.startswith(
        f"VectorResult(namespace='{tests.test_prefix}client_test', offset=0, next_cursor='"
    )
    # Random cursor string in the middle
    assert set_str.endswith(
        "', data=VectorColumns(ids=[7], vectors=[[0.7, 0.7]], attributes={'hello': ['world']}))"
    )
    ...
    for i, row in enumerate(results):
        if "test" in row.attributes and row.attributes["test"] == "cols":
            assert row == tpuf.VectorRow(
                id=vec_ids[i],
                vector=[i / 10, i / 10],
                attributes={"key1": key1[i], "key2": key2[i], "test": "cols"},
            )
        else:
            assert row == tpuf.VectorRow(
                id=vec_ids[i],
...
def test_force_index():
    ns = tpuf.Namespace(tests.test_prefix + "client_test-cosine")
    # Test upsert multiple dict rows
    ns.upsert(
        [
            {
    ...
    ns.force_index("cosine_distance")
